...
def main():
    ...
    with conn:
        with codecs.open('rest_category.csv', 'r', 'utf-8') as f:
            csv_reader = csv.reader(f, delimiter=';')
            for row in csv_reader:
                insert_rest_category(conn, row)
        with codecs.open('restaurant.csv', 'r', 'utf-8') as f:
            csv_reader = csv.reader(f, delimiter=';')
            for row in csv_reader:
                insert_restaurant(conn, row)
        with codecs.open('prod_category.csv', 'r', 'utf-8') as f:
            csv_reader = csv.reader(f, delimiter=';')
            for row in csv_reader:
                insert_product_category(conn, row)
        with codecs.open('product.csv', 'r', 'utf-8') as f:
            csv_reader = csv.reader(f, delimiter=';')
            for row in csv_reader:
